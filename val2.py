...
from datasets.body21 import Body21ValDataset
from models.with_mobilenet import PoseEstimationWithMobileNet
from modules.keypoints_2 import extract_keypoints, group_keypoints
...
def convert_to_coco_format(pose_entries, all_keypoints):
    ...
    for n in range(len(pose_entries)):
        if len(pose_entries[n]) == 0:
            continue
        keypoints = [0] * 17 * 3
        to_coco_map = [0,-1,6,8,10,5,7,9,-1,12,14,16,11,13,15,2,1,4,3,-1,-1]
...
def infer(net, img, scales, base_height, stride,
    ...
    avg_heatmaps = np.zeros((height, width, 22), dtype=np.float32)
    avg_pafs = np.zeros((height, width, 44), dtype=np.float32)
...
def evaluate(labels, output_name, images_folder, net, multiscale=False, visualize=False):
    ...
    dataset = Body21ValDataset(labels, images_folder)
    coco_result = []
    for sample in dataset:
        ...
        for kpt_idx in range(21):  # 22th for bg
...
if __name__ == '__main__':
    ...
    net = PoseEstimationWithMobileNet(num_heatmaps=22, num_pafs=44)
