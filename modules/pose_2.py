...
from modules.keypoints_2 import BODY_PARTS_KPT_IDS, BODY_PARTS_PAF_IDS
...
class Pose:
    ...
    def __init__(self, keypoints, confidence):
        ...
        self.id = None
        self.filters = [[OneEuroFilter(freq=30, mincutoff=1, beta=0.05), \
                         OneEuroFilter(freq=30, mincutoff=1, beta=0.05)] \
                         for _ in range(Pose.num_kpts)]
        ...
        if smooth:
            for kpt_id in range(Pose.num_kpts):
                ...
                current_pose.keypoints[kpt_id, 0] = \
                    current_pose.filters[kpt_id][0](current_pose.keypoints[kpt_id, 0])
                current_pose.keypoints[kpt_id, 1] = \
                    current_pose.filters[kpt_id][1](current_pose.keypoints[kpt_id, 1])
