...
from datasets.body25 import Body25TrainDataset
from datasets.transformations import ConvertKeypoints1, Scale, Rotate, CropPad, Flip1
...
from val1 import evaluate

cv2.setNumThreads(0)
cv2.ocl.setUseOpenCL(False)  # To prevent freeze of DataLoader

def train(prepared_train_labels, train_images_folder, num_refinement_stages, base_lr, batch_size,
          batches_per_iter, num_workers, checkpoint_path, weights_only, from_mobilenet, checkpoints_folder,
          log_after, val_labels, val_images_folder, val_output_name, checkpoint_after, val_after):
    net = PoseEstimationWithMobileNet(num_refinement_stages, num_heatmaps=26, num_pafs=52)

    stride = 8
    sigma = 7
    path_thickness = 1
    dataset = Body25TrainDataset(prepared_train_labels, train_images_folder,
    ...
    for epochId in range(current_epoch, 280):
        ...
        for batch_data in train_loader:
            ...
            for loss_idx in range(len(total_losses) // 2):
                losses.append(l2_loss(stages_output[loss_idx * 2], keypoint_maps, keypoint_masks,
                                      images.shape[0]))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prepared-train-labels', type=str, default='data/prepared_train_annotation_1.pkl',
                        help='path to the file with prepared annotations')
    parser.add_argument('--train-images-folder', type=str, default='coco/train2017/',
    ...
    parser.add_argument('--batches-per-iter', type=int, default=4,
                        help='number of batches to accumulate gradient from')
    ...
    parser.add_argument('--val-labels', type=str, default='data/val_subset_1.json',
                        help='path to json with keypoints val labels')
    parser.add_argument('--val-images-folder', type=str, default='coco/val2017/',
                        help='path to COCO val images folder')
    parser.add_argument('--val-output-name', type=str, default='data/detections.json',
    ...
    train(args.prepared_train_labels, args.train_images_folder, args.num_refinement_stages, args.base_lr,
          args.batch_size, args.batches_per_iter, args.num_workers, args.checkpoint_path, args.weights_only,
          args.from_mobilenet, checkpoints_folder, args.log_after, args.val_labels, args.val_images_folder,
          args.val_output_name, args.checkpoint_after, args.val_after)